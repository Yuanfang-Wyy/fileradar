...
| 数据库 | better-sqlite3 | ^9 | SQLite，FTS5 全文索引 |
...
- [ ] `db.ts`：SQLite 连接、Schema 初始化、FTS5 触发器
- [ ] `indexer.ts`：全量扫描、Batch insert、进度回调
- [ ] `watcher.ts`：chokidar 监听、增量 add/change/unlink
- [ ] `searcher.ts`：FTS5 查询、分页、筛选条件拼接
...
**当前阶段**：Phase 2 — 数据层（未开始）  
**最后更新**：Phase 1 完成 —— `pnpm dev` 可启动无边框窗口、typecheck 全绿。实施中的几处约定外决策：① preload 放在 `src/main/preload.ts`（沿用 shared/main/renderer 三目录约定，不新增 src/preload）；② pnpm 11.9 改用 `pnpm-workspace.yaml` 的 `allowBuilds` 放行 electron/esbuild；③ `electron-builder` 推迟至 Phase 6 安装（避开 app-builder-bin 大包超时）；④ `@electron/rebuild` 与 `better-sqlite3` 原生编译推迟至 Phase 2；⑤ 安装 electron 二进制经 `ELECTRON_MIRROR=npmmirror` 加速
...
4. **better-sqlite3 原生模块**：需要针对 Electron 版本重新编译（`electron-rebuild`），构建配置中必须处理
